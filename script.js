...
function createSlotButton(slotNumber) {
  ...
  button.setAttribute(
    "aria-label",
    `Open outlet slot ${formatSlot(slotNumber)}`
  );
...
function loadCheckout() {
  ...
  checkoutSlot.textContent =
    formatSlot(slotNumber);

  if (summaryCode) {
    summaryCode.textContent =
      `MINTO / ${formatSlot(slotNumber)}`;
  }
